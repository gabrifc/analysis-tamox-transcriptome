...
txMapping <- read.csv("txMapping.csv", 
                      row.names=1, sep=";", stringsAsFactors=FALSE)

## Update GO terms from Ensembl Biomart.
goTermsFile <- 'zfishGO.txt'
...
zfish2human <- read.csv("zfin.txt", skip = 1,
...
## Load DESeq2 
##----------------------------------------------------------------------------
sampleGrouping <- read.csv('sampleGrouping.csv', sep=";", row.names = 1)
...
QuaternaryProdData <- merge(x = resTamMapped,
                            y = zfish2human,
                            by.x = 'zfin_id_id',
                            by.y = 'ZFIN.ID') %>%
  dplyr::mutate(entrez = Gene.ID, 
                pvalue = svalue,
                fc = log2FoldChange) %>%
  dplyr::select(entrez, pvalue, fc) %>%
  dplyr::filter(complete.cases(.))
  
QuaternaryProdData <- deleteDuplicatesDataFrame(QuaternaryProdData, 'entrez')

quaternary_results <- RunCRE_HSAStringDB(QuaternaryProdData, 
                                         method = "Quaternary",
                                         fc.thresh = log2(0.01), 
                                         pval.thresh = 0.005,
                                         ...
                                         relations = NULL, 
                                         entities = NULL)
head(quaternary_results, n = 50)
quaternary_results$pvalue <- ifelse(quaternary_results$pvalue == -1, 1, quaternary_results$pvalue)
quaternary_results$padj <- p.adjust(quaternary_results$pvalue, method="BH")

write.csv2(quaternary_results, file = 'upstream_TAM_uninfected_vs_DMSO_uninfected.csv')
